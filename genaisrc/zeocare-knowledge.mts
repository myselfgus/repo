// HEALTH/HEALTH Knowledge Assistant - Um assistente especializado para o projeto HEALTH/HEALTH
// Este script cria um assistente capaz de responder perguntas detalhadas sobre o framework HEALTH/HEALTH

// Função para responder perguntas sobre os fundamentos do HEALTH/HEALTH
async function explicarFundamentos(query?: string) {
  // Verificar se query é uma string antes de usar métodos de string
  const queryStr = query && typeof query === 'string' ? query : '';
  
  const response = await $`
  Como especialista no framework HEALTH/HEALTH (Visualização INtegrativa TRAjetorial), responderei sua pergunta sobre os fundamentos do sistema:
  
  "${queryStr}"
  
  Basearei minha resposta nos princípios fundamentais do HEALTH/HEALTH:
  
  1. Abordagem Dimensional-Vetorial: O HEALTH/HEALTH representa experiências mentais como posições em um espaço multidimensional contínuo, rompendo com diagnósticos psiquiátricos puramente categoriais.
  
  2. Materialismo Existencial: Integra indissociavelmente a vivência subjetiva e o substrato material, transcendendo a falsa dicotomia entre abordagens materialistas e existencialistas.
  
  3. Metadimensões Fundamentais: Estrutura a experiência mental em três metadimensões (Emocional, Cognitiva, Autonomia) com 10 dimensões específicas.
  
  4. Trajetorialidade: Mapeia a evolução temporal do estado mental como um caminho no espaço dimensional, permitindo análise dinâmica.
  
  5. Visualização Significativa: Traduz dados dimensionais em visualizações intuitivas para uso clínico, destacando o radar dimensional como ferramenta central.
  `;
  
  return response;
}

// Função para explicar as dimensões do HEALTH/HEALTH
async function explicarDimensoes(query?: string) {
  // Verificar se query é uma string antes de usar métodos de string
  const queryStr = query && typeof query === 'string' ? query : '';
  
  const response = await $`
  Como especialista no framework HEALTH/HEALTH, vou explicar detalhadamente as dimensões que você está questionando:
